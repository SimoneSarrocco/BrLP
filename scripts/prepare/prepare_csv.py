...
def make_csv_B(df):
    ...
    sorting_field ='AgeatVisit' if 'AgeatVisit' in df.columns else 'AgeatConsent'
    
    data = []
    for subject_id in tqdm(df.Patient_ID.unique()):
        subject_df = df[ df.Patient_ID == subject_id ].sort_values(sorting_field, ascending=True)
        for i in range(len(subject_df)):
            for j in range(i+1, len(subject_df)):
                s_rec = subject_df.iloc[i]
                e_rec = subject_df.iloc[j]
                record = { 'subject_id': s_rec.Patient_ID, 'Gender': s_rec.Gender, 'split': s_rec.split }
